...
`parse_mode: HTML` with a compact `message` template: firing/resolved and severity emojis, then alertname, severity/service, `📌 Summary:`, description (when set, e.g. live error-rate %), and runbook + Prometheus source as `click-me` links (not a raw label dump). Helm list-merge replaces `receivers`, so keep that `message` block in `telegram.local.yaml`.
