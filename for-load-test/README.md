...
**Grafana → folder `sla-slo-sli`** — **SLA / SLO / SLI — demo-load** (`uid: slo-demo-load`). SLI is 5xx/total on `demo_http_requests_total`. **SLO is error rate < 1% over 5m** so the ~5% `/error` mix **fires** `DemoLoadErrorRateSLOBreach`. Runbook: Grafana folder **Runbooks**. Public links use `lab.publicBaseURL` in [`helmfile/values/lab.yaml`](../helmfile/values/lab.yaml) (do not hardcode the LB IP). See [helmfile/README.md](../helmfile/README.md). Portfolio walkthrough (screenshots): [`../demo/sla-incident/`](../demo/sla-incident/).
