...
- `helmfile/` — local charts + values + releases (same idea as chatbot-infra-helm). One `helmfile sync`: presync hook applies `traefik.io` CRDs, then Traefik, then official Kubernetes Dashboard `7.14.0` (vendored; upstream archived, helm repo 404).
