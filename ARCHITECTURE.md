...
- `helmfile/` — local charts + values + releases (same idea as chatbot-infra-helm). One `helmfile sync`: presync hook applies `traefik.io` CRDs, then Traefik, official Kubernetes Dashboard `7.14.0` (vendored; upstream archived, helm repo 404), then Jaeger all-in-one + OpenTelemetry Collector (OTLP → Jaeger).
