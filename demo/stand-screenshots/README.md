...
Install proof: live UIs from the lab after `helmfile sync`. One folder per service (Grafana dashboard and, where it exists, the service UI). Open a folder, or browse below.

SLA/SLO incident walkthrough (breach → alert → runbook → logs → traces → recovery): [`../sla-incident/`](../sla-incident/).
