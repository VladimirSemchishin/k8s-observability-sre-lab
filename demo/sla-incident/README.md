...
1. **Grafana + Prometheus** — собирают и отображают метрики со всего, что развёрнуто в Kubernetes, через дашборды, в том числе SLA/SLO/SLI, сущности k8s (node, pod, network, pv, ... etc).
2. **Alertmanager** — уведомления по критическим метрикам и сообщение в Telegram со всей информацией для быстрой реакции.
...
![Путь инженера: HTTP 500 в кластере → SLO recovered](incident-flow.png)
...
Это общий дашборд, в котором описаны SLA/SLO/SLI для конкретного сервиса (в примере это `demo-load`). 
Он показывает фактические показатели SLI: по нему сразу видно, выполняются обязательства или нет.

![Grafana SLO dashboard — SLI above 1%](01-slo-dashboard-breach.png)

## Шаг 2 — Алерт (Telegram / Alertmanager)

Инженер получает уведомление в удобный мессенджер — в примере это Telegram — с краткой сводкой: 
- что случилось,
- какая метрика,
- где лежит runbook (как чинить и куда смотреть)
- ссылка на эту метрику в Prometheus.
Это позволяет получить весь нужный контекст за минимальное время.
...
Из алерта инженер открывает runbook (инструкция именно по этому alert). В ней уже написано, какой дашборд смотреть, как найти ошибку в логах, как починить. Особенно полезно в ночных дежрствах.

![Grafana Runbooks — DemoLoadErrorRateSLOBreach](04-runbook.png)

## Шаг 4 — Логи (Loki)

В runbook обычно есть ссылка на место где смотреть логи.
Пока инцидент живой, в Loki видны логи приложения: статус **500**, путь **`/error`**, и в той же JSON-строке **`trace_id`**. Это ответ на вопрос «что именно падает прямо сейчас», без `kubectl logs`.

![Grafana Loki — demo /error with trace_id](05-grafana-logs.png)

## Шаг 5 — Трейсы (Jaeger)

`trace_id` из лога открывается в Jaeger: видно, что ломается конкретный запрос **`GET /error`**, а соседние `GET /work` / `GET /slow` отрабатывают. Так инженер может определить где именно проблема.
